...
Shows TOP 5 results for EVERY test (not just failures).
"""
...
def query_llm_for_answer(query: str, top_results: List[Dict[str, Any]], openai_client) -> str:
    ...
    for i, result in enumerate(top_results[:5], 1):
        ...
        text = result.get('text', 'No text available')[:300]
        
        context_parts.append(
            f"Result {i}: {peripheral}\n"
            f"Content: {text}\n"
        )
...
def run_tests(use_reranker: bool = False, use_llm: bool = False):
    """Run all test cases and report results - ALWAYS SHOW TOP 5."""
    print("\n" + "=" * 80)
    print("SVD REGISTER RETRIEVAL TEST SUITE - VERBOSE MODE")
    ...
    for test in TEST_CASES:
        ...
        # ALWAYS show results for every test
        print(f"\n{'=' * 80}")
        ...
        for i, r in enumerate(results[:5], 1):
            ...
            if debug:
                ...
                # Show what boosts/penalties were applied
                ...
                if applied.get('register_match'):
                    print(f"       ✓ Register boost applied")
                if applied.get('peripheral_penalty'):
                    print(f"       ⚠️  Peripheral penalty applied")
...
if __name__ == "__main__":
    ...
    success = run_tests(use_reranker=use_reranker, use_llm=use_llm)
